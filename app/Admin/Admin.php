...
class FlexyPress_Scroll_To_Top_Menu {
    ...
    public function register_sub_menu() {
        add_submenu_page( 
            'themes.php', 
            __( 'FP Scroll to Top', 'flexypress-scroll-to-top' ), 
            __( 'FP Scroll to Top', 'flexypress-scroll-to-top' ), 
            'manage_options', 
    ...
    public function inline_styles() {
        ...
        if ( $custom_css ) { 
            wp_register_style( 'fp_scroll_to_top_inline_css', false, [], '1.0' );
    ...
    public function render_form() {
        if (
            isset($_POST['FlexyPress_Scroll_To_Top_Menu']) &&
            $_POST['FlexyPress_Scroll_To_Top_Menu'] === 'FlexyPress_Scroll_To_Top_Menu' &&
            isset($_POST['flexypress_scroll_to_top_nonce_field']) &&
            wp_verify_nonce($_POST['flexypress_scroll_to_top_nonce_field'], 'flexypress_scroll_to_top_nonce')
        ) {
            if (!empty($_POST['fp_scroll_type'])) {
                update_option('fp_scroll_to_top_type', sanitize_text_field(wp_unslash($_POST['fp_scroll_type'])));
            }
    
            if (!empty($_POST['fp_scroll_btn_text'])) {
                update_option('fp_scroll_to_top_btn_text', sanitize_text_field(wp_unslash($_POST['fp_scroll_btn_text'])));
            }
    
            if (!empty($_POST['fp_scroll_svg_url'])) {
                update_option('fp_scroll_to_top_svg_url', sanitize_textarea_field(wp_unslash($_POST['fp_scroll_svg_url'])));
            }
    
            if (!empty($_POST['fp_scroll_side'])) {
                update_option('fp_scroll_to_top_side', sanitize_text_field(wp_unslash($_POST['fp_scroll_side'])));
            }
    
            if (!empty($_POST['fp_scroll_color'])) {
                update_option('fp_scroll_to_top_color', sanitize_text_field(wp_unslash($_POST['fp_scroll_color'])));
            }
    
            if (!empty($_POST['fp_scroll_background'])) {
                update_option('fp_scroll_to_top_background', sanitize_text_field(wp_unslash($_POST['fp_scroll_background'])));
            }
    
            if (!empty($_POST['fp_scroll_radius'])) {
                update_option('fp_scroll_to_top_radius', sanitize_text_field(wp_unslash($_POST['fp_scroll_radius'])));
            }
    
            if (!empty($_POST['fp_scroll_css'])) {
                update_option('fp_scroll_to_top_css', sanitize_textarea_field(wp_unslash($_POST['fp_scroll_css'])));
            } else {
